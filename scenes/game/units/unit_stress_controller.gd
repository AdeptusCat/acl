...
@export var pin_threshold: float = 35.0          
# effective stress level at which squad becomes "Pinned"
@export var panic_threshold: float = 65.0        
# effective stress level at which squad enters "Panic"
@export var recovery_bias: float = 0.85          
# bias factor to make recovery easier once already stressed
# (>0 tilts decay rates so it’s faster to rally back down than to climb further up)
...
func _maybe_transition(delta: float) -> void:
	...
	match state:
		...
		MoraleState.CAUTIOUS:
			...
			elif S_eff >= pin_threshold and roll < _rate_to_prob(0.5, delta):
				_set_state(MoraleState.PINNED)
		...
		MoraleState.PANIC:
			if S_eff < pin_threshold * 0.7 and roll < _rate_to_prob(0.28 * recovery_bias, delta):
				_set_state(MoraleState.CAUTIOUS)
